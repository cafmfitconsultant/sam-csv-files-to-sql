...
```bash
sam-edne-to-sql-parser$ sam build
```
...
```bash
sam-edne-to-sql-parser$ sam logs -n StockCheckerFunction --stack-name sam-edne-to-sql-parser --tail
```
...
```bash
sam-edne-to-sql-parser$ cd functions/stock-checker
...
```bash
aws cloudformation delete-stack --stack-name sam-edne-to-sql-parser
```
